...
/// @class Level2Scene
/// @brief Représente la scène du Flag
...
public:
  ...
  void transitLine(std::map<string, Vector2i> transition, std::map<string, Vector2i> destination, std::map<string, Line*> components){
    ...
    Engine::getInstance().resetSizeWindow(Vector2i(950,540));
		projection.loadProjection(Engine::getInstance().getWidth(), Engine::getInstance().getHeight());
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glMultMatrixf(projection);

    VisualComponentsFinal["1Banner"] = new Image(Vector2i(MathUtils::getCenteredPositionX(Engine::getInstance().getWidth(), 800), 25), Vector2i(800, 150), engine.assetManager.getAsset<Texture*>("Dialog"));
    
    //First Dialog
		LabelGrid *firstDialog = new LabelGrid(Vector2i(0,0), Vector2i(0,0), 0);
		Label *dialog0 = new Label(Vector2i(0,0), font24, "Wow, you made it here !!", dialogText);
		firstDialog->addLabel(dialog0);

		//First Dialog
		LabelGrid *secondDialog = new LabelGrid(Vector2i(0,0), Vector2i(0,0), 0);
		Label *dialog1 = new Label(Vector2i(0,0), font24, "I knew you could do it !", dialogText);
		secondDialog->addLabel(dialog1);
		
		//First Dialog
		LabelGrid *thirdDialog = new LabelGrid(Vector2i(0,0), Vector2i(0,0), 0);
		Label *dialog2 = new Label(Vector2i(0,0), font24, "Now all you got to do is press [v] and enjoy the magic.", dialogText);
		thirdDialog->addLabel(dialog2);

    VisualComponentsFinal["2firstDialog"] = firstDialog;
		VisualComponentsFinal["2secondDialog"] = secondDialog;
		VisualComponentsFinal["2thirdDialog"] = thirdDialog;

    firstDialog->setPosition(MathUtils::getCenteredPosition(VisualComponentsFinal["1Banner"]->getSize(), firstDialog->getSize()) + VisualComponentsFinal["1Banner"]->getPos());
		firstDialog->resetPosition();

		secondDialog->setPosition(MathUtils::getCenteredPosition(VisualComponentsFinal["1Banner"]->getSize(), secondDialog->getSize()) + VisualComponentsFinal["1Banner"]->getPos());
		secondDialog->resetPosition();

		thirdDialog->setPosition(MathUtils::getCenteredPosition(VisualComponentsFinal["1Banner"]->getSize(), thirdDialog->getSize()) + VisualComponentsFinal["1Banner"]->getPos());
    ...
    dialogManager = new DialogManager(3, ((Image*)VisualComponentsFinal["1Banner"]));
